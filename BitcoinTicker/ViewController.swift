...
//  BitcoinTicker
//
//  Created by Johit Garg on 15/03/2018.
//  Copyright © 2018 Johit Garg. All rights reserved.
//
...
//    
    func getBitcoinData(url: String, row:Int) {
        let currencySymbol = ["$","R$","$","￥","€","￡","$","Rp","She","₹","￥","$","kr","$","zl","lei","P","kr","$","$","R"]
        Alamofire.request(url, method: .get)
            .responseJSON { response in
                if response.result.isSuccess {
                    ...
                    print(bitcoinJSON)
                    self.updateBitcoinData(json: bitcoinJSON,symbol:currencySymbol[row])
